...
def parse_netherworld(html: str) -> Dict[str, CharSnapshot]:
    ...
    for tr in rows:
        ...
        if not m:
            continue
        exp_pct = float(m.group(1))  # ".08" -> 0.08
...
def write_error(run_id: str, character: str, message: str):
    with ERRORS_CSV.open("a", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow([run_id, format_ts(now_berlin()), f"{character} Fehler {message}"])
...
def post_discord_run(run_id: str, dungeon: str, stage: int, difficulty: str, spot: str,
                     party_type: str, started_at: str, ended_at: str,
                     rows: List[Tuple[str, Optional[int], Optional[float], Optional[int], Optional[float], Optional[float]]]):
